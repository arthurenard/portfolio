...
export default function SnakePage() {
  ...
  const reset = useCallback(() => {
    const s = [{ x: 10, y: 10 }];
    stateRef.current = {
      snake: s, dir: { x: 1, y: 0 }, nextDir: { x: 1, y: 0 },
      food: spawnFood(s), score: 0, alive: true, started: false,
    };
    setScore(0); setDead(false); setStarted(false);
  }, []);

  const steer = useCallback((dirKey: string) => {
    const state = stateRef.current;
    if (!state.started) { state.started = true; setStarted(true); }
    const d = DIRS[dirKey];
    if (!d || (d.x === -state.dir.x && d.y === -state.dir.y)) return;
    state.nextDir = d;
  }, []);
  ...
  const draw = useCallback(() => {
    ...
    ctx.strokeStyle = dark ? "#1f2937" : "#e5e7eb"; ctx.lineWidth = 0.5;
  ...
  const tick = useCallback((ts: number) => {
    const state = stateRef.current;
    if (!state.alive || !state.started) { draw(); rafRef.current = requestAnimationFrame(tick); return; }
    if (ts - lastTickRef.current >= TICK) {
      lastTickRef.current = ts;
      state.dir = state.nextDir;
      const head = state.snake[0];
      const next = { x: (head.x+state.dir.x+COLS)%COLS, y: (head.y+state.dir.y+ROWS)%ROWS };
      if (state.snake.some((s) => s.x===next.x && s.y===next.y)) {
        state.alive = false; setDead(true); draw(); return;
      }
      state.snake.unshift(next);
      if (next.x===state.food.x && next.y===state.food.y) {
        state.score++; setScore(state.score); state.food = spawnFood(state.snake);
  ...
  const DPadBtn = ({ dir, label }: { dir: string; label: string }) => (
    <button
      ...
      className="w-14 h-14 flex items-center justify-center rounded-xl bg-gray-200 dark:bg-gray-700 active:bg-gray-300 dark:active:bg-gray-600 text-gray-700 dark:text-gray-200 text-xl font-bold select-none touch-none"
    >
      {label}
    </button>
  );
    ...
    <main className="min-h-screen relative pt-20 pb-24 flex flex-col items-center">
      <div className="w-full max-w-lg px-4">
        ...
        <div className="relative">
          <canvas ref={canvasRef} width={W} height={H} className="w-full rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm"/>
          {!started && !dead && (
            <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/40 backdrop-blur-sm">
              <p className="text-white text-base font-medium text-center px-4">
                {isMobile ? "Use the buttons below to start" : "Press any arrow key to start"}
              </p>
            </div>
          )}
          {dead && (
            <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl bg-black/50 backdrop-blur-sm gap-4">
              ...
              <button onClick={reset} className="px-6 py-2 bg-white text-gray-900 rounded-lg font-medium hover:bg-gray-100 transition-colors">
                Play again
              </button>
            </div>
          )}
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-4 text-center">
          {isMobile ? "Tap the d-pad to move" : "Arrow keys or WASD · Wraps around edges"}
        </p>
      </div>
    </main>
  );
